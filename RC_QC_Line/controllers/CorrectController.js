// Controller for handling Lot correction
...
class CorrectController {
  ...
  async requestOldLot(userId, replyToken) {
    try {
      // Set user state to waiting for old Lot
  ...
  async processOldLot(userId, oldLot, replyToken) {
    try {
      ...
      await this.requestNewLot(userId, oldLot.trim(), replyToken);
    } catch (error) {
  ...
  // Request new Lot number
  async requestNewLot(userId, oldLot, replyToken) {
    try {
      // Set user state to waiting for new Lot
  ...
  async processNewLot(userId, oldLot, newLot, replyToken) {
    try {
      ...
      // Correct the Lot
      await this.correctLot(userId, oldLot, newLot.trim(), replyToken);
    } catch (error) {
  ...
  // Correct Lot number
  async correctLot(userId, oldLot, newLot, replyToken) {
    try {
      // Validate inputs
    ...
    try {
      ...
      try {
        ...
        const updateResult = await new sql.Request(transaction).query`
          UPDATE Images
          SET lot_id = ${newLotId}, updated_at = GETDATE()
        ...
        const updatedCount = updateResult.recordset[0].count;
